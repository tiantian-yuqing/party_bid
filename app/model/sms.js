...
//            if(Bidding.message_is_valuable(json_message.messages[0])){
...
//                        if(_(activity.bids).findWhere({bid_state:2} == undefined)){
//                           native_accessor.send_sms(json_message.messages[0].phone, '对不起，竞价尚未开始！');
//                        }
//                        else{
//                            native_accessor.send_sms(json_message.messages[0].phone, '对不起，竞价已结束！');
//                        }
//                    }
//                    else{
//                        if(_(activity.bids.JJ_list).findWhere({phone:json_message.messages[0].phone} == undefined)){
...
//                   console.log(_(activity.bids).findWhere({bid_state:1}));
//                   console.log(_(activity.bids).findWhere({bid_state:1}) == undefined);
                    if (_(activity.bids).findWhere({bid_state:1}) == undefined){
                        send_message = _(activity.bids).findWhere({bid_state:2}) == undefined? '对不起，竞价尚未开始！':'对不起，竞价已结束！';
                    }
                    else{
                       // console.log(activity.bids.JJ_list);
                      //  console.log(_(activity.bids.JJ_list).findWhere({phone:json_message.messages[0].phone}) == undefined);
//                        send_message = _(activity.bids.JJ_list).findWhere({phone:json_message.messages[0].phone}) == undefined?'恭喜！您已出价成功':'您已成功出价，请勿重复出价！'
              ...
              if( send_message == '恭喜！您已出价成功'){
                ...
                //  console.log(activity_object);
                 ...
                 // console.log(JSON.parse( localStorage.getItem('activity_object')));
                  location.reload(true);
              }
